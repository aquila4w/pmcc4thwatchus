...
function addSecurityHeaders(response: NextResponse) {
  ...
  response.headers.set(
    ...
    [
      ...
      "media-src 'self' https://images.pmcc4thwatch.us",
      "frame-src https://maps.google.com/ https://www.google.com/maps/ https://www.zeffy.com/ https://www.google.com/recaptcha/",
      "connect-src 'self' data: blob: https://www.google-analytics.com https://api.qrserver.com https://api.zeffy.com",
      "object-src 'none'",
      "base-uri 'self'",
      "form-action 'self'",
    ].join("; ")
...
export function middleware(request: NextRequest) {
  // Cloudflare Worker passes original subdomain via __host param or X-Forwarded-Host header.
  // Also check direct Host header for local dev and non-proxied requests.
  const proxyHost = request.nextUrl.searchParams.get("__host")
    || request.headers.get("x-forwarded-host");
  const host = proxyHost || request.headers.get("host") || "";
...
export const config = {
  matcher: ["/((?!api|payload-api|_next/static|_next/image|favicon.ico|cms/static).*)"],
};
