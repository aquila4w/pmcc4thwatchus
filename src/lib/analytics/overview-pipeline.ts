          ...
          counts: [
            {
              $group: {
                _id: null,
                totalScans: { $sum: 1 },
                memberScans: { $sum: { $cond: [{ $eq: ["$inviteType", "member"] }, 1, 0] } },
                churchScans: { $sum: { $cond: [{ $eq: ["$inviteType", "church"] }, 1, 0] } },
                platformScans: { $sum: { $cond: [{ $eq: ["$inviteType", "platform"] }, 1, 0] } },
                registeredFromScans: { $sum: { $cond: [{ $eq: ["$registered", true] }, 1, 0] } },
              },
            },
          ],
          timeline: [
            ...
            },
            {
              $group: {
                _id: "$_id.date",
                memberScans: { $sum: { $cond: [{ $eq: ["$_id.type", "member"] }, "$count", 0] } },
                churchScans: { $sum: { $cond: [{ $eq: ["$_id.type", "church"] }, "$count", 0] } },
                platformScans: { $sum: { $cond: [{ $eq: ["$_id.type", "platform"] }, "$count", 0] } },
